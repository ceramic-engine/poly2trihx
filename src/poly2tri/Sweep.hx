...
class Sweep
{
    ...
    inline public function sweepPoints() 
    {
    ...
    inline public function finalizationPolygon() 
    {
     ...
     * will be filled to.
     */
    inline public function pointEvent(point:Point) :Node
    {
        // trace(point);
    ...
    inline public function edgeEventByEdge(edge:Edge, node:Node) 
    {
    ...
    inline public function edgeEventByPoints(ep:Point, eq:Point, triangle:Triangle, point:Point) 
    {
    ...
    inline public function newFrontTriangle(point:Point, node:Node) :Node
    {
        var triangle = Pool.getTriangle(point, node.point, node.next.point);

        // trace(node.id);
        // trace(node.triangle);
        triangle.markNeighborTriangle(node.triangle);

        var new_node = Pool.getNode(point);
     ...
     */
    inline public function fill(node:Node) 
    {
        var triangle = Pool.getTriangle(node.prev.point, node.point, node.next.point);
     ...
     */
    inline public function fillAdvancingFront(n:Node)
    {
        var node:Node;
        var angle:Float;
     ...
     */
    inline public function fillBasin(node:Node) 
    {
    ...
    inline public function isShallow(node:Node):Bool 
    {
    ...
    inline public function fillEdgeEvent(edge:Edge, node:Node) 
    {
